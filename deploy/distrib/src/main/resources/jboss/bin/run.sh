...
#
# Helper to stop the H2 server
#
stop_db_server() {
    H2_PID_FILE="$1"
    if [ -s $H2_PID_FILE ]; then
       H2_PID=`cat "$H2_PID_FILE"`
       kill -0 $H2_PID 2> /dev/null
       if [ $? -eq 0 ]; then
           # The process still exists try to kill it
           kill $H2_PID

           # Ensure H2 is stopped
           tries=0
           kill -0 $H2_PID 2> /dev/null
           while [ $? -eq 0 -a $tries -lt 5 ]; do
               sleep 1s
               let tries+=1
               kill -0 $H2_PID 2> /dev/null
           done
...
# Setup Bonita specific properties
BONITA_DATABASES_DIR="${BONITA_HOME}/server/platform/work/databases"
mkdir -p ${BONITA_DATABASES_DIR}
...
echo ""
echo "  BONITA_HOME: $BONITA_HOME"
echo ""
echo "========================================================================="
echo ""

# Start the H2 server in TCP mode before JBoss so there is no error about "port already in use"
# with the 2 datasources for Bonita

# If H2 is still from the previous session, kill it
H2_PID_FILE="${BONITA_DATABASES_DIR}/h2.pid"

stop_db_server "$H2_PID_FILE"

if [ "x$BONITA_H2_SERVER" = "xtrue" ]; then
    echo "Starting H2 server listening on TCP port ${BONITA_H2_PORT}"
    "$JAVA" \
      -classpath "$JBOSS_HOME/server/default/lib/h2-1.3.170.jar" \
      org.h2.tools.Server \
      -tcp -tcpAllowOthers -tcpPort ${BONITA_H2_PORT} -baseDir "${BONITA_DATABASES_DIR}" 2>&1 >> ${BONITA_DATABASES_DIR}/h2.log  &
    echo $! > $H2_PID_FILE
    echo "H2 PID (`cat $H2_PID_FILE`) stored in ${H2_PID_FILE}"
fi
