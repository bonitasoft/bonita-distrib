'use strict';

describe('Process auto generated form', function () {

  var $rootScope, scope, $location, contractSrvcMock, urlParserMock, $window, $q, deferedProcessStart, $httpBackend, createController;

  beforeEach(module('autogeneratedForm'));

  beforeEach(inject(function ($injector, $controller) {

    $rootScope = $injector.get('$rootScope');
    scope = $rootScope.$new();
    $q = $injector.get('$q');
    $httpBackend = $injector.get('$httpBackend');
    contractSrvcMock = jasmine.createSpyObj('contractSrvc', ['fetchContract', 'startProcess']);
    urlParserMock = jasmine.createSpyObj('urlParser', ['getQueryStringParamValue']);
    $location = $injector.get('$location');
    $window = $injector.get('$window');
    var i18nServiceMock = jasmine.createSpyObj('i18nService', ['then']);
    i18nServiceMock.then.and.callFake(function() {
      scope.i18nLoaded = true;
    });
    $httpBackend.expect('GET', '../API/system/session/unusedId').respond([{}]);
    spyOn($window.location, 'assign');
    spyOn($window.parent, 'postMessage');
    urlParserMock.getQueryStringParamValue.and.returnValue('1');
    var processAPIMock = {
      get: function(parameters, success, error) {
        return success({id:1});
      }
    };
    contractSrvcMock.fetchContract.and.callFake(function() {
      var deferred = $q.defer();
      deferred.resolve({data:'{}', status: 200});
      return deferred.promise;
    });
    deferedProcessStart = $q.defer();
    contractSrvcMock.startProcess.and.callFake(function() {
      return deferedProcessStart.promise;
    });

    createController = function() {
      return $controller('MainCtrl', {
        '$scope': scope,
        'contractSrvc': contractSrvcMock,
        'urlParser': urlParserMock,
        '$window': $window,
        'processAPI': processAPIMock,
        'i18nService': i18nServiceMock
      });
    }
  }));

  describe('postMessage', function () {

    var currentWindow;

    var stringifiedJSONArgsMatcher = function(actualArgs, expectedArgs) {
      for (var i= 0; i < actualArgs.length; i++) {
        var argBeforeStringify;
        try {
          argBeforeStringify = JSON.parse(actualArgs[i]);
        } catch (e) {
          argBeforeStringify = actualArgs[i];
        }
        if (!jasmine.matchersUtil.equals(argBeforeStringify, expectedArgs[i])) {
          return false;
        }
      }
      return true;
    };

    beforeEach(function () {
      currentWindow = $window.self;

      createController();
      $httpBackend.flush();
      $rootScope.$digest();
    });

    afterEach(function () {
      $window.self = currentWindow;
    });

    it('should be sent on success', function () {
      jasmine.addCustomEqualityTester(stringifiedJSONArgsMatcher);
      $window.self = null;
      var responseStatus = 200;
      deferedProcessStart.resolve({data:'{"caseId":1}', status: responseStatus});

      scope.postData();

      $rootScope.$digest();

      expect($window.parent.postMessage).toHaveBeenCalledWith(jasmine.objectContaining(
        {
          message: 'success',
          status: responseStatus,
          action: 'Start process',
          targetUrlOnSuccess: '/bonita'
        }), '*');
    });

    it('should be sent on error', function () {
      jasmine.addCustomEqualityTester(stringifiedJSONArgsMatcher);
      $window.self = null;
      var responseStatus = 500;
      deferedProcessStart.reject({data:'FileTooBigError', status: responseStatus});

      scope.postData();

      $rootScope.$digest();

      expect($window.parent.postMessage).toHaveBeenCalledWith(jasmine.objectContaining(
        {
          message: 'error',
          status: responseStatus,
          action: 'Start process',
          targetUrlOnSuccess: '/bonita'
        }), '*');
    });

    it('should not be sent if not in an iframe', function () {
      $window.self = $window.parent;

      scope.postData();

      $rootScope.$digest();

      expect($window.parent.postMessage).not.toHaveBeenCalled();
    });
  });
});
